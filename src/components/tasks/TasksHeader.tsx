  ...
  return <>
      ...
      <div className="flex items-center justify-between p-4 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="relative mt-7">
            <Popover open={displayPopoverOpen} onOpenChange={setDisplayPopoverOpen}>
              <PopoverTrigger asChild>
                <Button className="w-12 h-12 rounded-full transition-all duration-500 hover:scale-105" style={{
                backgroundColor: '#2e2e30'
              }} size="icon">
                  <LayoutGrid className="h-5 w-5 text-white" />
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-[300px] h-auto max-h-[500px] p-4 border border-[#414141] shadow-xl overflow-y-auto rounded-[20px]" style={{
              background: '#1F1F1F'
            }} align="start">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    padding: '0px'
                      ...
                      return <button key={view.value} onClick={() => {
                  ...
                  <div>
                    <Collapsible open={!sortCollapsed} onOpenChange={open => setSortCollapsed(!open)}>
                      ...
                      <CollapsibleContent className="space-y-3">
                        ...
                        label: 'Chats',
                        icon: MessageSquare
                      }].map(sortOption => <div key={sortOption.key} className="flex items-center justify-between">
                            <span className="text-gray-300 text-sm">{sortOption.label}</span>
                            <IconToggle icon={sortOption.icon} checked={sortSettings[sortOption.key as keyof typeof sortSettings]} onCheckedChange={checked => {
                          const newSettings = {
                            ...sortSettings,
                            [sortOption.key]: checked
                          };
                  ...
                  <div>
                    <Collapsible open={!filterCollapsed} onOpenChange={open => setFilterCollapsed(!open)}>
                      ...
                      <CollapsibleContent className="space-y-4">
                        {/* Date Filter */}
                        <InlineDateFilter isActive={filterSettings.date} selectedDate={filterValues.date} onToggle={checked => {
                        const newSettings = {
                          ...filterSettings,
                          date: checked
                        };
                        ...
                        {/* Priority Filter */}
                        <InlinePriorityFilter isActive={filterSettings.priority} selectedPriorities={filterValues.priorities} onToggle={checked => {
                        const newSettings = {
                          ...filterSettings,
                          priority: checked
                        };
                        ...
                        {/* Label Filter */}
                        <InlineLabelFilter isActive={filterSettings.label} selectedLabels={filterValues.labels} onToggle={checked => {
                        const newSettings = {
                          ...filterSettings,
                          label: checked
                        };
          ...
          <div className="relative mt-7">
            <Button onClick={onCreateTask} className="w-12 h-12 rounded-full transition-all duration-500 hover:scale-105" style={{
            backgroundColor: '#2e2e30'
          }} size="icon">
        ...
        <div className="flex items-center gap-6 mt-7">
          <div onClick={() => setCurrentTaskView('total')} className="flex flex-col items-center group cursor-pointer transition-all duration-300 px-[3px]">
          ...
          <div onClick={() => setCurrentTaskView('drafts')} className="flex flex-col items-center group cursor-pointer transition-all duration-300 px-[10px]">
            <span className={`font-orbitron text-2xl font-bold ${currentTaskView === 'drafts' ? 'border-b-2' : ''}`} style={{
            color: '#C4B5FD',
            borderColor: currentTaskView === 'drafts' ? '#C4B5FD' : 'transparent'
          }}>
              {draftTasks}
            </span>
            <span className="text-xs text-gray-400 font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          ...
          <div onClick={() => setCurrentTaskView('pending')} className="flex flex-col items-center group cursor-pointer transition-all duration-300">
            <span className={`font-orbitron text-2xl font-bold ${currentTaskView === 'pending' ? 'border-b-2' : ''}`} style={{
            color: '#f59e0b',
            borderColor: currentTaskView === 'pending' ? '#f59e0b' : 'transparent'
          }}>
              {pendingTasks}
            </span>
            <span className="text-xs text-gray-400 font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          ...
          <div onClick={() => setCurrentTaskView('completed')} className="flex flex-col items-center group cursor-pointer transition-all duration-300">
            <span className={`font-orbitron text-2xl font-bold ${currentTaskView === 'completed' ? 'border-b-2' : ''}`} style={{
            color: '#10B981',
            borderColor: currentTaskView === 'completed' ? '#10B981' : 'transparent'
          }}>
              {completedTasks}
            </span>
            <span className="text-xs text-gray-400 font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          ...
          <div onClick={() => setCurrentTaskView('deleted')} className="flex flex-col items-center group cursor-pointer transition-all duration-300">
            <span className={`font-orbitron text-2xl font-bold ${currentTaskView === 'deleted' ? 'border-b-2' : ''}`} style={{
            color: '#F87171',
            borderColor: currentTaskView === 'deleted' ? '#F87171' : 'transparent'
          }}>
              {deletedTasks}
            </span>
            <span className="text-xs text-gray-400 font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-200">
      ...
      <div className="px-4 mt-[20px]">
        <div className="ml-20">
          <h1 className="text-white font-semibold" style={{
          fontSize: '3.5rem'
        }}>
            {currentTaskView === 'total' ? 'Tasks' : <span>
